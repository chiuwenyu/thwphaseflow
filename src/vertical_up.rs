...
impl VerticalUp {
    ...
    fn SimilarityAnalysis(&mut self) {
        ...
        // Calculate Result
        self.RL = 1.0 - Rg;
        let LoTP = self.LoL * lamda.powf(2.0) / (1.0 - Rg) + self.LoG * (1.0 - lamda).powf(2.0) / Rg; // Eq. (29)
        ...
        let fTP = (1.0 + LnLanda / (1.281 - 0.478 * LnLanda + 0.444 * LnLanda.powf(2.0) -
            0.094 * LnLanda.powf(3.0) + 0.00843 * LnLanda.powf(4.0))) * f0; // Eq. (31)

        self.Pfric = fTP * LoTP * (ULS + UGS).powf(2.0) / (2.0 * G * self.ID) / 10000.0 * 100.0 * self.SF; // Eq. (32)
    ...
    fn BubbleModel(&mut self) {
        ...
        while delta > eps {
            let U0 = (1.0f64 - alfa).sqrt() * 1.53 * ((self.ST * G * (self.LoL - self.LoG)) / (self.LoL * self.LoL)).powf(0.25); // Eq. (38)
